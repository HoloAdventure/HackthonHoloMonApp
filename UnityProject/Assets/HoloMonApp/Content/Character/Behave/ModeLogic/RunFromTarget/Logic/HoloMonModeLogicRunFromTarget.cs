...
{
    public class HoloMonModeLogicRunFromTarget : MonoBehaviour, HoloMonActionModeLogicIF
    {
        /// <summary>
        /// モードロジック共通参照
        /// </summary>
        ...
        /// 固有アクションデータの参照
        /// </summary>
        private ModeLogicRunFromTargetData p_Data =>
            (ModeLogicRunFromTargetData)p_ModeLogicCommon.ModeLogicSetting.ModeLogicData;


        /// <summary>
        /// 現在逃走中のオブジェクト
        /// </summary>
        [SerializeField, Tooltip("現在逃走中のオブジェクト")]
        private GameObject p_RunFromObject;

        [SerializeField, Tooltip("逃走対象を確認する時間間隔(秒)")]
        float p_TargetCheckTimeSpan = 1.0f;
        float p_TargetCheckTimeElapsed;

        [SerializeField, Tooltip("逃走対象が見当たらない場合に追跡を解除する上限回数")]
        int p_TargetNotFoundThreshold = 3;

        [SerializeField, Tooltip("逃走対象が見当たらなかった回数")]
            ...
            if (p_ModeLogicCommon.ModeLogicStatus == HoloMonActionModeStatus.Runtime)
            {
                ...
                if (p_TargetCheckTimeElapsed >= p_TargetCheckTimeSpan)
                {
                    ...
                    VisionObjectWrap friendObjectWrap = p_ModeLogicReference
                        ...
                        .CheckCollectionByGameObject(p_RunFromObject);
                    ...
                    if (friendObjectWrap == null)
                    {
                        ...
                        {
                            // 指定回数見つからなければ失敗として逃走を解除する
                            Debug.Log("RunFromTarget Missing : Target Not Found");
                            StopMode(new ModeLogicResult(HoloMonActionModeStatus.Missing, new ModeLogicRunFromTargetReturn()));
                            return;
                        }
                    }
                    else
                ...
                // ターゲットの座標
                Vector3 targetPosition = p_RunFromObject.transform.position;

                // ターゲットまでの距離が停止距離か判定する
                if (CheckStoppingDistance(targetPosition, selfPostion))
                {
                    // ターゲット位置に到達していれば完了とする
                    Debug.Log("RunFrom Achievement");
                    StopMode(new ModeLogicResult(HoloMonActionModeStatus.Achievement, new ModeLogicRunFromTargetReturn()));
                    return;
                }
                ...
                {
                    // 到達場所が停止距離以遠の状態でスタックしていれば逃走失敗と判定する
                    Debug.Log("RunFrom Missing");
                    StopMode(new ModeLogicResult(HoloMonActionModeStatus.Missing, new ModeLogicRunFromTargetReturn()));
                    return;
                }

                // 逃走ターゲットの座標を取得する
                Vector3 runFromTargetPosition = RunTargetPosition(targetPosition, selfPostion);

                // 逃走ターゲットの足元座標 (Y軸方向の追跡はしないので軸の座標は自身と同じとする)
                Vector3 runFromTargetFootPosition = new Vector3(
                    runFromTargetPosition.x,
                    selfPostion.y,
                    runFromTargetPosition.z
                    );

                // アニメーションパラメータに速度設定を反映する
                // (実速度では変動が大きいため、設定値をそのまま利用する)
                ApplyAnimationSpeed(p_Data.RunSetting.MoveSpeed);
                ...
                // ターゲットへの方向ベクトル
                Vector3 toTargetVector = (runFromTargetFootPosition - selfPostion).normalized;

                // フレーム間隔に合わせて回転速度と移動速度を計算する
                float deltaRotateSpeed = p_Data.RunSetting.RotateSpeed * Time.deltaTime;
                float deltaMoveSpeed = p_Data.RunSetting.MoveSpeed * Time.deltaTime;
                ...
                // 移動ベクトルを位置に反映する
                Vector3 nextPosition = Vector3.MoveTowards(selfPostion, runFromTargetFootPosition, deltaMoveSpeed);
        ...
        private bool EnableSetting()
        {
            ...
            // 逃走オブジェクトを設定する
            p_RunFromObject = p_Data.TargetObject;
        ...
        private bool CheckStoppingDistance(Vector3 a_TargetPosition, Vector3 a_SelfPostion)
        {
            ...
            switch (p_Data.RunSetting.RunCheckPosition)
            {
                case RunFromTargetCheckPosition.ObjectOrigin:
                    // オリジナルの座標で判定
                    arrivalCheckPosition = a_TargetPosition;
                    break;
                case RunFromTargetCheckPosition.FootPosition:
            ...
            float checkDistance = p_Data.RunSetting.StoppingDistance + CurrentActualForwardDepthLength();
